...
from src.game import Color, GameResult, GomokuGame
from src.logger import GameLogger
from src.network import DQNAgent, QNetwork
...
def get_reward(game_result, agent_is_black, game, action, agent_color, opponent_color):
    ...
    return calculate_shaped_reward(game, action, agent_color, opponent_color)
...
class HeuristicAgent:
    ...
    def _pattern_score(self, game, row, col, color):
        total = 0
        open_fours = semi_open_fours = 0
        open_threes = semi_open_threes = 0
        open_twos = semi_open_twos = 0
        for dr, dc in DIRECTIONS:
            ...
            if length == 4:
                ...
                if both_open:
                    ...
                    semi_open_twos += 1
        total += open_fours * self.SCORE_OPEN_4 + semi_open_fours * self.SCORE_SEMI_OPEN_4
        total += open_threes * self.SCORE_OPEN_3 + semi_open_threes * self.SCORE_SEMI_OPEN_3
        total += open_twos * self.SCORE_OPEN_2 + semi_open_twos * self.SCORE_SEMI_OPEN_2
        return total
    ...
    def _lookahead_score(self, game, candidate):
        ...
        if cloned.result != GameResult.ONGOING:
            we_win = cloned.result == (GameResult.BLACK_WIN if self.color == Color.BLACK.value else GameResult.WHITE_WIN)
            return 1e9 if we_win else -1e9
    ...
    def select_action(self, game):
        ...
        if k > 1 and best_score < self.SCORE_WIN and best_score < self.SCORE_BLOCK_WIN:
            lookahead_scores = [(self._lookahead_score(game, move), move) for _, move in top]
    ...
    def update_model(self):
        ...
        if models:
            ...
            try:
                ...
                print(f"Could not load self-play model {chosen_model}: {e}")
...
def play_episode(player, opponent, agent_is_black):
    ...
    while game.result == GameResult.ONGOING:
        ...
        if is_agent_turn:
            ...
            game.step(action)
            step_reward = calculate_shaped_reward(game, action, agent_color.value, opponent_color.value)
            if game.result != GameResult.ONGOING:
                final_reward = Config.WIN_REWARD if game.result != GameResult.DRAW else Config.DRAW_REWARD
                episode_transitions.append((state_before_move, action_int, final_reward, None, True))
            else:
            ...
            if game.result != GameResult.ONGOING:
                ...
                episode_transitions.append((last_agent_state, last_agent_action, final_reward, None, True))
...
def train():
    ...
    for episode in trange(Config.TOTAL_EPISODES):
        ...
        if progress < 0.20:
            ...
            opponent = random_opponent if random.random() < 0.80 else HeuristicAgent(opponent_color)
        elif progress < 0.50:
            opponent = random_opponent if random.random() < 0.50 else HeuristicAgent(opponent_color)
        elif progress < 0.70:
            opponent = HeuristicAgent(opponent_color)
        else:
            if self_play_opponent.has_model and random.random() < 0.50:
        ...
        rolling_win_rate = sum(rolling_window) / len(rolling_window) if rolling_window else 0.0
        ...
        logger.log_game(result, num_moves, metadata={"episode": episode, "agent_color": "BLACK" if agent_is_black else "WHITE"})
        ...
        if (episode + 1) % Config.SAVE_FREQ == 0:
            ...
            print(f"  Epsilon: {player.epsilon:.3f}, Loss: {loss_str}, Buffer: {len(player.buffer)}")
            logger.save()
